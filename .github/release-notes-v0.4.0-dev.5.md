# VoxHearth 0.4.0 Development Preview 5
...
- Performance signposts now identify overlapping operations independently, and
  ...
  configuration.
- The sealed Metal build now uses an explicit target and is compiled twice in
  the protected check; both the library and manifest must be byte-identical.
...
2. Download `VoxHearth-v0.4.0-dev.5-unsigned.dmg` and `SHA256SUMS` from this
