# VoxHearth 0.4.0 Development Preview 6
...
- Performance signposts now identify overlapping operations independently, and
  ...
  configuration.
- The sealed Metal build uses an explicit target and is compiled twice in the
  protected check; both the library and manifest must be byte-identical.
  Preview-cancellation coverage uses an explicit inference gate instead of a
  scheduler-sensitive timing window.
- Release automation distinguishes semantic correctness on variable shared
  runners from performance acceptance on the documented M2/16 GB floor. The
  shipping app retains its 2-second cleanup deadline; CI's longer bounded
  evaluator deadline does not alter product behavior.
...
2. Download `VoxHearth-v0.4.0-dev.6-unsigned.dmg` and `SHA256SUMS` from this
