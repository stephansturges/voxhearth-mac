...
// Residency sets require runtime availability dispatch and a background
// keepalive worker. VoxHearth keeps them disabled for a bounded static runtime.
...
ggml_metal_library_t ggml_metal_library_init(ggml_metal_device_t dev) {
    ...
    NSString * resource_root = [bundle resourcePath];
    NSString * path = [resource_root stringByAppendingPathComponent:@"Metal/ggml-llama.metallib"];

    NSError * error = nil;
    NSDictionary * attributes = [[NSFileManager defaultManager] attributesOfItemAtPath:path error:&error];
    if (error != nil || attributes == nil || attributes[NSFileType] != NSFileTypeRegular) {
        GGML_LOG_ERROR("%s: sealed Metal library is unavailable\n", __func__);
        return nil;
    }
    ...
    if (error != nil || library == nil) {
        GGML_LOG_ERROR("%s: sealed Metal library could not be loaded\n", __func__);
        return nil;
    }

    ggml_metal_library_t result = calloc(1, sizeof(struct ggml_metal_library));
    if (result == nil) {
        [library release];
        return nil;
    }
    ...
    result->obj = library;
    result->dev = dev;
    result->pipelines = ggml_metal_pipelines_init();
    result->lock = [NSLock new];
    return result;
}

ggml_metal_library_t ggml_metal_library_init_from_source(
        ggml_metal_device_t device,
        const char * source,
        bool verbose) {
    // Runtime source compilation is deliberately absent from VoxHearth.
    GGML_UNUSED(device);
    GGML_UNUSED(source);
    GGML_UNUSED(verbose);
    return NULL;
...
ggml_metal_device_t ggml_metal_device_init(int device) {
    ...
    if (dev->mtl_device == nil) {
        ...
        if (dev->mtl_device) {
            ...
            if (![[dev->mtl_device name] containsString:@"M5"] &&
            ...
            // Tensor and bfloat capability are derived from the device family.
            // The sealed metallib contains both paths; no runtime shader probe is allowed.
