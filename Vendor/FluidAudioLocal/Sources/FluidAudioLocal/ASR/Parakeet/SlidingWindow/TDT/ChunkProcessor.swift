...
// Modified for VoxHearth: 2026-08-10. Replaced the disk-capable sample-source
// abstraction with an in-memory Float array owned by the transcription call.
...
struct ChunkProcessor {
    ...
    init(audioSamples: [Float]) {
        ...
        self.totalSamples = audioSamples.count
    }
    ...
    func readSamples(offset: Int, count: Int) throws -> [Float] {
        guard
            offset >= 0,
            count >= 0,
            offset <= audioSamples.count,
            count <= audioSamples.count - offset
        else {
            throw ASRError.invalidAudioData
        }
        return Array(audioSamples[offset..<(offset + count)])
    }
