...
- [ ] Create an annotated, signed tag `v0.1.0` on that commit and push it.

The release workflow rejects any tag other than `v0.1.0` and rejects a tag that
does not point at the checked-out commit. It runs on `macos-26` with Xcode 26.2,
fetches and verifies the model, builds and signs the app, notarizes and staples
