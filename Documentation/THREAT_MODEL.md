...
This threat model applies to the signed VoxHearth 0.1 release built from this
repository. It distinguishes the installed runtime, which is designed for
offline operation, from the connected build and release system.
...
| Global hotkey becomes a keylogger | Carbon registers one configured hotkey and receives only its press/release events. | Accessibility permission is powerful; a compromised VoxHearth binary could misuse it. Signature and source verification matter. |
